...
// Fetch projects
export const fetchProjects = async (page: number = 1, pageSize: number = 25): Promise<ProjectsResponse> => {
	try {
		const response = await apiClient.get<Project[]>(`/projects?page=${page}&pageSize=${pageSize}`);
...
// Delete project
export const deleteProject = async (projectId: number): Promise<ApiResponse<any>> => {
	try {
...
// Update project
export const updateProject = async (projectId: number, data: Partial<Project>): Promise<ApiResponse<Project>> => {
	try {
